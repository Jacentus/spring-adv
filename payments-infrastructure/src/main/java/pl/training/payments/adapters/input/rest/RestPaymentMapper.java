...
public interface RestPaymentMapper {
    ...
    @Mapping(target = "amount", expression = "java(new Money(BigDecimal.valueOf(chargeRequestDto.getAmount()), Currency.getInstance(chargeRequestDto.getCurrencyCode())))")
    ...
    @Mapping(target = "amount", expression = "java(transaction.money().amount().toString() + ' ' + transaction.money().currency().getCurrencyCode())")
